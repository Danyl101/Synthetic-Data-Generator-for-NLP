...
def datasplit():
    content_folder=os.path.join(os.path.dirname(os.path.abspath(__file__)),'..',
                                config['paths']['bert']['raw_text_data']['cleaned_data_folder']) #Path of content directory
    
    base_dir='Datasets'
    os.makedirs(base_dir,exist_ok=True)
    
    # Create these folders if they don't exist
    for folder in ['train_folder', 'val_folder', 'test_folder']: #Creates directories
        path=os.path.join(base_dir,folder)
        os.makedirs(path,exist_ok=True)
    
    filelist=[f for f in os.listdir(content_folder) if f.endswith('txt')] # Collects files drom directory
    ...
    train_files=filelist[:train_end]  #Splits files across directories
    val_files=filelist[train_end:val_end]
    test_files=filelist[val_end:]
    
    for f in train_files: #Copies files into directories
